...
@section('content')
    <div class="card">
        <div class="card-header">
            <p><strong>Edit Testimoni</strong></p>
        </div>
        <div class="card-body">
            <form action="{{ route('admin.testimoni.update', $testimoni->id) }}" method="POST">
                    ...
                    <select class="form-select" id="kerjaan_id" name="kerjaan_id" required>
                        ...
                        @foreach ($kerjaans as $kerjaan)
                            <option value="{{ $kerjaan->id }}" data-user-id="{{ $kerjaan->user_id }}"
                                {{ (old('kerjaan_id') ?? $testimoni->kerjaan_id) == $kerjaan->id ? 'selected' : '' }}>
                    ...
                    <select class="form-select" id="user_id" name="user_id" required>
                        ...
                        @foreach ($users as $user)
                            <option value="{{ $user->id }}"
                                {{ (old('user_id') ?? $testimoni->user_id) == $user->id ? 'selected' : '' }}>
                                {{ $user->name }}</option>
                        @endforeach
                    </select>
                    @error('user_id')
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control" id="deskripsi" name="deskripsi" rows="3" required>{{ old('deskripsi') ?? $testimoni->deskripsi }}</textarea>
                    @error('deskripsi')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control" id="rating" name="rating" min="1" max="5"
                        value="{{ old('rating') ?? $testimoni->rating }}" required>
                    @error('rating')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
